...
export const registerHandler = async (req, rep) => {
...
export const loginHandler = async (req, rep) => {
    const { email, password, tenantId } = loginSchema.parse(req.body);
    const token = await req.jwt.sign({ sub: 'mock_user_id', tenant: tenantId, roles: ['user'] }, { expiresIn: '1h' });
